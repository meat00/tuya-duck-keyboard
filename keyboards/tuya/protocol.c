...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_T01, DP_TYPE_STRING},
};

unsigned char duck_flag = false;
unsigned char duck_val;
...
void uart_transmit_output(unsigned char value)
{
    //#error "请将MCU串口发送函数填入该函数,并删除该行"
	  //HAL_UART_Transmit(&huart1, &value, 1, 0x200);
      uart_tx(&value, 1);
/*
    //Example:
...
void all_data_update(void)
{
    ...
    //mcu_dp_value_update(DPID_TEMP_CURRENT,20); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_HUMIDITY_VALUE,40); //VALUE型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_t01_handle
功能描述 : 针对DPID_T01的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_t01_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_T01,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_T01:
            // test处理函数
            ret = dp_download_t01_handle(value,length);
        break;
